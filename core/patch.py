...
def make_patch(old_context, new_context):
    ...
    while i < len(old_context) or j < len(new_context):
        if (
            k < len(lcs)
            and i < len(old_context)
            and j < len(new_context)
            and old_context[i] == lcs[k]
            and new_context[j] == lcs[k]
        ):
            patch.append(KeepOp())
            i += 1
            j += 1
